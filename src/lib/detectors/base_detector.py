...
class BaseDetector(object):
    def __init__(self, opt):
        ...
        self.model = create_model(opt.arch, opt.heads, opt.head_conv, opt.fpn)
    ...
    def run(self, image_or_path_or_tensor, meta=None):
        ...
        for scale in self.scales:
            ...
            if not pre_processed:
                ...
                meta['gt'] = pre_processed_images['meta'].get('gt', None)
            ...
            output, dets, forward_time = self.process(
                images, gt=meta.get('gt', None), return_time=True
            )
            ...
            for i in range(self.opt.num_fpn):
                dets[i] = self.post_process(dets[i], meta, scale, fpn_stride=pow(2, self.opt.num_fpn - i - 1))
                detections.append(dets[i])
            torch.cuda.synchronize()
        ...
        if self.opt.debug == 3:
            ...
            if isinstance(image_or_path_or_tensor, str):
                prefix = osp.splitext(osp.basename(image_or_path_or_tensor))[0]
                genID = False
            else:
                img_info = image_or_path_or_tensor['img_info']
                prefix = osp.splitext(osp.basename(img_info['file_name']))[0]
                crop_str = list(map(str, img_info['crop']))
                prefix = '_'.join([prefix] + crop_str)
                genID = True
            self.save_results(debugger, image, results, path=self.opt.debug_dir, prefix=prefix, genID=genID)
