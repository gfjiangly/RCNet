...
def ctdet_decode(heat, wh, reg=None, a=None, cat_spec_wh=False, K=100, a_method=0, debug=0):
    batch, cat, height, width = heat.size()

    if debug > 0:
        heat_array = heat.cpu().numpy()
    ...
    if a is not None:
        ...
        if a_method == 0:
            ...
            a = a.detach().cpu().numpy().squeeze() * 180. - 90.
    ...
    if a is not None:
        ...
        #     new_bboxes.append(cv2.boxPoints((center_np[i], wh_np[i], a[i])).reshape(-1))
        # bboxes = torch.from_numpy(np.array(new_bboxes)).float().cuda().unsqueeze(0)
...
def trans_gt(anns, output, in_shape, num_cls=15, a_method=1):
    ...
    # wh = torch.zeros((1, 2, new_h, new_w), dtype=torch.float32).cuda()
    # angle = torch.zeros((1, 1, new_h, new_w), dtype=torch.float32).cuda()
    # offset = torch.zeros((1, 2, new_h, new_w), dtype=torch.float32).cuda()
    wh = output['wh']
    angle = output['a']
    offset = output['reg']
    for ann in anns:
        ...
        try:
            ...
            y_start = ct_int[1] - 2 if ct_int[1] - 2 >= 0 else ct_int[1]
            y_end = ct_int[1] + 2 if ct_int[1] + 2 < new_h else ct_int[1]
            x_start = ct_int[0] - 2 if ct_int[0] - 2 >= 0 else ct_int[0]
            x_end = ct_int[0] + 2 if ct_int[0] + 2 < new_w else ct_int[0]
            for y_i in range(y_start, y_end):
                for x_j in range(x_start, x_end):
                    wh[0][0][y_i][x_j] = w / down_ratio_w
                    wh[0][1][y_i][x_j] = h / down_ratio_h
                    angle[0][0][y_i][x_j] = a / 90.
            # wh[0][0][ct_int[1]][ct_int[0]] = w / down_ratio_w
            # wh[0][1][ct_int[1]][ct_int[0]] = h / down_ratio_h
            # angle[0][0][ct_int[1]][ct_int[0]] = a / 90.
        except IndexError:
            print(ann)
    return hm, wh, angle, offset


def ctdet_decode_replace(heat, wh, opt, reg=None, a=None,
                         gt_c=None, gt_wh=None, gt_a=None):
    batch, cat, height, width = heat.size()
    ...
    if reg is not None:
        reg = _tranpose_and_gather_feat(reg, inds)
        reg = reg.view(batch, opt.K, 2)
        xs = xs.view(batch, opt.K, 1) + reg[:, :, 0:1]
        ys = ys.view(batch, opt.K, 1) + reg[:, :, 1:2]
    else:
        xs = xs.view(batch, opt.K, 1) + 0.5
        ys = ys.view(batch, opt.K, 1) + 0.5
    wh = _tranpose_and_gather_feat(wh, inds)
    if opt.cat_spec_wh:
        wh = wh.view(batch, opt.K, cat, 2)
        clses_ind = clses.view(batch, opt.K, 1, 1).expand(batch, opt.K, 1, 2).long()
        wh = wh.gather(2, clses_ind).view(batch, opt.K, 2)
    else:
        wh = wh.view(batch, opt.K, 2)
    if a is not None:
        a = _tranpose_and_gather_feat(a, inds)
        if opt.a_method == 0:
            a = a.detach().cpu().numpy().squeeze() * (-90.)
        ...
        # new_wh = torch.where(a > 0, wh[..., [1, 0]], wh[..., [0, 1]])
        # new_a = torch.where(a > 0., a - 1., a)
    clses = clses.view(batch, opt.K, 1).float()
    scores = scores.view(batch, opt.K, 1)
    bboxes = torch.cat([xs - wh[..., 0:1] / 2,
                        ys - wh[..., 1:2] / 2,
                        xs + wh[..., 0:1] / 2,
                        ys + wh[..., 1:2] / 2], dim=2)
