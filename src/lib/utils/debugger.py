...
class Debugger(object):
    ...
    def add_loss(self, loss_stats):
        try:
            for l in loss_stats:
                if l not in self.losses:
                    self.losses[l] = [loss_stats[l].mean().item()]
                else:
                    self.losses[l].append(loss_stats[l].mean().item())
        except Exception as e:
            print(loss_stats)
    
    def draw_loss(self, iter_id, to_file=None):
        import matplotlib.pyplot as plt
        import pylab as pl
        styles = ['g-', 'r-', 'b-', 'y-', 'm-']
        ...
        for name, loss in self.losses.items():
            pl.plot(np.arange(len(loss)), loss, styles[i], label=name)
            i += 1
        pl.legend()
        pl.xlabel(u'iters')
        pl.ylabel(u'loss')
        plt.title('Compare loss for different vars in training')
        if to_file:
            plt.savefig(to_file)
        else:
            plt.show()
    ...
    def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default', coor_len=4):
        ...
        if coor_len == 8:
            self.imgs[img_id] = cvtools.draw_boxes_texts(self.imgs[img_id], [bbox], [txt], box_format='polygon')
        elif coor_len == 4:
            cv2.rectangle(
    ...
    def add_bbox_for_paper(self, bbox, cat, img_id='default', coor_len=4):
        bbox = np.array(bbox, dtype=np.int32)
        # cat = (int(cat) + 1) % 80
        cat = int(cat)
        # print('cat', cat, self.names[cat])
        # c = self.colors[cat][0][0].tolist()
        colors = [[249, 47, 47], [53, 253, 8], [7, 233, 252], [235, 8, 249], [8, 22, 249],
                  [194, 249, 8], [249, 95, 8], [33, 8, 249], [249, 180, 8], [213, 249, 8],
                  [8, 249, 172], [8, 114, 249], [8, 114, 249], [8, 114, 249], [8, 114, 249]]
        c = colors[cat][::-1]
        # if self.theme == 'white':
        #     c = (255 - np.array(c)).tolist()
        if coor_len == 8:
            self.imgs[img_id] = cvtools.draw_boxes_texts(
                self.imgs[img_id], [bbox], colors=[c], line_width=2, box_format='polygon')
        elif coor_len == 4:
            cv2.rectangle(
                self.imgs[img_id], (bbox[0], bbox[1]), (bbox[2], bbox[3]), c, 2)
